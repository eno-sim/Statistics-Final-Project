...
```{r}
set.seed(123)
fit_tree <- rpart(TPY ~ NUMBED + SQRFOOT + MSA + URBAN + PRO + TAXEXEMPT + SELFFUNDINS + MCERT + ORGSTR, data=train, method = "anova", control = rpart.control(cp = 0))
...
```{r}
set.seed(123)
fit_tree <- rpart(TPY ~NUMBED + SQRFOOT + MSA + SELFFUNDINS + MCERT + ORGSTR, data=train, method = "anova")
printcp(fit_tree) #cp table
plotcp(fit_tree) #plot cp values, error, splits
...
```{r}
set.seed(123)
fit_tree <- rpart(TPY ~NUMBED + SQRFOOT + MSA + SELFFUNDINS + MCERT + ORGSTR, data=train, method = "anova", control = rpart.control(cp = 0.0017))
printcp(fit_tree) #cp table
plotcp(fit_tree) #plot cp values, error, splits
...
for (i in seq_along(ntree)) {
  set.seed(123)
  fit_bag <- bagging(TPY ~ NUMBED + SQRFOOT + MSA + URBAN + PRO + TAXEXEMPT + SELFFUNDINS + MCERT + ORGSTR, data=train, coob = TRUE, nbagg =        ntree[i])
  rmse[i] <-fit_bag$err
}
...
For the dataset the best number of trees is 16. \

Bagging with 16 trees, random split: \

```{r}
set.seed(123)

# train 
fit_bag16 <- bagging(TPY ~ NUMBED + SQRFOOT + MSA + URBAN + PRO + TAXEXEMPT + SELFFUNDINS + MCERT + ORGSTR, nbagg=28, data=train, coob = TRUE)
...
Bagging with 16 trees, 10 fold CV: \
...
# CV bagged model
fit_bag <- train(TPY ~ NUMBED + SQRFOOT + MSA + URBAN + PRO + TAXEXEMPT + SELFFUNDINS + MCERT + ORGSTR, data = train, method = "treebag", trControl = ctrl, importance = TRUE)
...
```{r}
set.seed(123)
fit_forest <- randomForest(TPY ~ NUMBED + SQRFOOT + URBAN + PRO + TAXEXEMPT + MSA + SELFFUNDINS + MCERT + ORGSTR, data = train, proximity=TRUE) 
plot(fit_forest)
