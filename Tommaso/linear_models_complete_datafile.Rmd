...
```{r}
ggpairs(subset(datafile, select = c(SQRFOOT, NUMBED, TPY)))
```
...
```{r}
summary(lm(TPY ~ NUMBED + SQRFOOT + NUMBED:SQRFOOT, data = datafile))
```
...
```{r}
anova(glm(TPY ~ NUMBED + SQRFOOT + NUMBED:SQRFOOT, data = datafile, family = gaussian),
      test = "Chisq")
```
...
```{r}
AIC(lm(TPY ~ NUMBED, data = datafile),
    lm(TPY ~ SQRFOOT, data = datafile),
    lm(TPY ~ NUMBED + SQRFOOT, data = datafile),
    lm(TPY ~ NUMBED:SQRFOOT, data = datafile),
    lm(TPY ~ NUMBED + SQRFOOT + NUMBED:SQRFOOT, data = datafile),
    lm(TPY ~ NUMBED + NUMBED:SQRFOOT, data = datafile),
    lm(TPY ~ SQRFOOT + NUMBED:SQRFOOT, data = datafile))
```
...
```{r}
AIC(lm(TPY ~ log(NUMBED), data = datafile),
    lm(TPY ~ log(SQRFOOT), data = datafile),
    lm(TPY ~ log(NUMBED) + log(SQRFOOT), data = datafile),
    lm(TPY ~ log(NUMBED) + SQRFOOT, data = datafile),
    lm(TPY ~ NUMBED + log(SQRFOOT), data = datafile))
```
...
```{r}
fit.linear <- lm(TPY ~ NUMBED + SQRFOOT, data = datafile)
summary(fit.linear)
```
```{r}
par(mfrow = c(2, 2))
...
```{r}
par(mfrow = c(2, 2))
plot(lm(TPY ~ NUMBED, data = datafile))
```
```{r}
par(mfrow = c(2, 2))
plot(lm(TPY ~ NUMBED + SQRFOOT + NUMBED:SQRFOOT, data = datafile))
```
```{r}
par(mfrow = c(2, 2))
plot(lm(TPY ~ NUMBED + NUMBED:SQRFOOT, data = datafile))
```
```{r}
par(mfrow = c(2, 2))
plot(lm(TPY ~ NUMBED + log(SQRFOOT), data = datafile))
```
...
```{r}
par(mfrow = c(2, 2))
hist(datafile$TPY, xlab = "TPY", freq = TRUE)
hist(datafile$NUMBED, xlab = "NUMBED", freq = TRUE)
hist(datafile$SQRFOOT, xlab = "SQRFOOT", freq = TRUE)
```
...
library(GGally)
datafile$TPY_log <- log(datafile$TPY)
datafile$SQRFOOT_log <- log(datafile$SQRFOOT)
datafile$NUMBED_log <- log(datafile$NUMBED)

ggpairs(subset(datafile, select = c(TPY_log, SQRFOOT_log, NUMBED_log)))
...
```{r}
summary(lm(log(TPY) ~ log(NUMBED) + log(SQRFOOT) + log(NUMBED):log(SQRFOOT), data = datafile))
```

```{r}
anova(glm(log(TPY) ~ log(NUMBED) + log(SQRFOOT) + log(NUMBED):log(SQRFOOT), data = datafile,
...
```{r}
summary(lm(log(TPY) ~ log(NUMBED), data = datafile))
```
```{r}
summary(lm(log(TPY) ~ log(SQRFOOT), data = datafile))
```

And to their residuals:
```{r}
par(mfrow = c(2, 2))
plot(lm(log(TPY) ~ log(NUMBED), data = datafile))
```
```{r}
par(mfrow = c(2, 2))
plot(lm(log(TPY) ~ log(SQRFOOT), data = datafile))
```

We look at the AIC too:
```{r}
AIC(lm(log(TPY) ~ log(NUMBED), data = datafile),
    lm(log(TPY) ~ log(SQRFOOT), data = datafile),
    lm(log(TPY) ~ log(NUMBED) + log(SQRFOOT), data = datafile))
```
...
```{r}
summary(lm(log(TPY) ~ NUMBED + log(SQRFOOT), data = datafile))
```
```{r}
par(mfrow = c(2, 2))
plot(lm(log(TPY) ~ NUMBED + log(SQRFOOT), data = datafile))
```
```{r}
AIC(lm(log(TPY) ~ NUMBED + log(SQRFOOT), data = datafile))
```
...
```{r}
summary(lm(I(1/TPY) ~ NUMBED + SQRFOOT, data = datafile))
```
```{r}
summary(lm(I(sqrt(TPY)) ~ NUMBED + SQRFOOT, data = datafile))
```

```{r}
par(mfrow = c(2, 2))
plot(lm(I(1/TPY) ~ NUMBED + SQRFOOT, data = datafile))
```
```{r}
par(mfrow = c(2, 2))
plot(lm(I(sqrt(TPY)) ~ NUMBED + SQRFOOT, data = datafile))
```
...
```{r}
summary(lm(TPY ~ NUMBED + SQRFOOT + TAXEXEMPT + SELFFUNDINS + URBAN + PRO, data = datafile))
```
```{r}
anova(glm(TPY ~ NUMBED + SQRFOOT + TAXEXEMPT + SELFFUNDINS + URBAN + PRO, data = datafile,
          family = gaussian), test = "Chisq")
```
```{r}
par(mfrow = c(2, 2))
...
anova(glm(log(TPY) ~ log(NUMBED) + log(SQRFOOT) + TAXEXEMPT + SELFFUNDINS + URBAN + PRO,
          data = datafile, family = gaussian), test = "Chisq")
```
```{r}
par(mfrow = c(2, 2))
